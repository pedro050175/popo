...
class AlquilerRepository {
    ...
    public function extraer_todos(): ?array {
        ...
        foreach ($alquilerData as $data){
            $alquiler[] = Alquiler::fromArray($data);
        }
        return $alquileres;
    }
    public function save (array $alquiler):void {
        if (isset($alquiler['alquiler']['id'])) {
    ...
    public function update (array $alquiler): void{
        ...
        $this->conexion->consulta("UPDATE alquiler SET $changes where id =".$alquiler['alquiler']['id']); //vease que se cierra la consulta con " y se concatena con . Esto se hace porque
    ...
    public function read (int $id): ?Alquiler {
        $this->conexion->consulta("SELECT * FROM alquiler WHERE (id=$id)");
        return $this->extraer_registro();
    }
    public function delete (int $id): void {
       $this->conexion->consulta("DELETE FROM alquiler WHERE (id=$id)");
    }
    /* public function relacionados(int $id): bool {
        $encontrados=0; //he definido una constante de tipo array asociativo que contiene ["campo_tabla_relacionado_con_id_"=>'nombre_tabla']  
